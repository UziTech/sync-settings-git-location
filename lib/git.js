...
async function invalidGitBranch (invalidUrl) {
	...
	const notification = atom.notifications.addError((invalidUrl ? 'Invalid Git Branch' : 'No Git Branch'), {
		description: (invalidUrl
			? `Invalid Branch name: \`${invalidUrl}\`. Remember: the branch HAS TO already exist in your repository. Due to how git handles clones it is not possible to clone an empty repository with a specific branch name.`
		...
		buttons: [{
			...
			async onDidClick () {
				...
				const inputView = new InputView({
					...
					description: 'IMPORTANT: the branch HAS TO ALREADY EXIST!',
...
async function getGitBranch () {
	...
	if (gitBranch && gitBranch != '') {
		try {
            ...
            gitBranch = await invalidGitBranch(gitBranch)
            if (gitBranch) {
        		return gitUrl.trim()
        	}
		}
	} else {
		// no git branch in settings, go without it
        return ''
	}
}
...
async function cloneToLocalRepo (gitUrl, gitBranch) {
	...
	try {
        let gitCommand
        if (gitBranch != ''){
            gitCommand = `clone --branch ${gitBranch} --depth 1 ${gitUrl} .`
        } else {
            gitCommand = `clone --depth 1 ${gitUrl} .`
        }
		await git(gitCommand, repoPath, '', { env })
	} catch (ex) {
